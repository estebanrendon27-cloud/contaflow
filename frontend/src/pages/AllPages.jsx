...
export function OnboardingPage() {
  ...
  const { empresaService: empSvc } = { empresaService: require('../services/api').empresaService };

  const onSubmit = async (data) => {
    if (step < 4) { setStep(s => s + 1); return; }
    setLoad(true);
    try {
      const { empresaService } = await import('../services/api');
      await empresaService.crear(data);
      toast.success('¡Empresa configurada correctamente!');
      const { useEmpresaStore } = await import('../store');
      await useEmpresaStore.getState().cargarEmpresas();
      navigate('/dashboard');
    } catch (e) {
  ...
  const STEPS_LABELS = ['Tu empresa', 'Actividad', 'Token DIAN', 'Plan contable'];

  return (
    <div className="min-h-screen bg-[#0A0E1A] flex items-center justify-center p-4"
         style={{ backgroundImage:'linear-gradient(rgba(61,123,255,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(61,123,255,0.03) 1px,transparent 1px)', backgroundSize:'50px 50px' }}>
      <div className="w-full max-w-xl">
        {/* Logo */}
        <div className="flex items-center gap-2.5 mb-8 justify-center">
          <div className="w-9 h-9 rounded-xl bg-[#00E5B0] flex items-center justify-center font-black text-[#0A0E1A]">CF</div>
          ...
          {STEPS_LABELS.map((l, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border
                transition-all
                ${i+1 < step  ? 'bg-[#00E5B0] border-[#00E5B0] text-[#0A0E1A]'
                : i+1 === step ? 'border-[#00E5B0] text-[#00E5B0]'
                : 'border-white/15 text-[#8892AA]'}`}>
                {i+1 < step ? '✓' : i+1}
              </div>
              <div className={`text-[9px] uppercase tracking-wider
                ${i+1 === step ? 'text-[#00E5B0]' : 'text-[#8892AA]'}`}>{l}</div>
            </div>
          ))}
        </div>

        <div className="bg-[#141C2E] border border-white/7 rounded-2xl p-6">
          <form onSubmit={handleSubmit(onSubmit)}>
            {step === 1 && (
              <StepEmpresa register={register} errors={errors} watch={watch} />
            )}
            {step === 2 && <StepActividad register={register} />}
            {step === 3 && <StepToken />}
            {step === 4 && <StepPUC register={register} />}

            <div className="flex items-center justify-between mt-6">
              {step > 1 ? (
                <button type="button" onClick={() => setStep(s => s-1)}
                        className="text-sm text-[#8892AA] hover:text-[#F0F4FF] flex items-center gap-1">
                  ← Atrás
                </button>
              ) : <div />}
              <button type="submit" disabled={loading}
                      className="flex items-center gap-2 bg-[#00E5B0] text-[#0A0E1A] px-6 py-2.5
                                 rounded-xl text-sm font-semibold hover:shadow-[0_6px_20px_rgba(0,229,176,0.3)]
                                 disabled:opacity-50 transition-all">
                {loading ? <Loader2 size={14} className="animate-spin"/> : null}
                {step < 4 ? 'Continuar →' : '🎉 Finalizar configuración'}
              </button>
            </div>
          </form>
        </div>
...
function StepEmpresa({ register, errors, watch }) {
  const nit = watch('nit','');
  // Calcular DV del NIT
  const calcDV = (n) => {
    ...
    let sum = 0;
    for (let i=0;i<clean.length;i++) sum += parseInt(clean[clean.length-1-i]) * arr[i];
    const r = sum % 11;
    return r > 1 ? 11-r : r;
  };
  ...
  return (
    <div>
      <h2 className="text-xl font-black mb-1 text-[#F0F4FF]" style={{ fontFamily:'Syne,sans-serif' }}>Datos de tu empresa</h2>
      <p className="text-sm text-[#8892AA] mb-5">NIT, razón social y régimen tributario</p>
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="col-span-2">
          <label className="block text-xs text-[#8892AA] mb-1">NIT *</label>
          <input placeholder="900.123.456" className={inputBaseCls}
                 {...register('nit', { required: 'Requerido' })} />
        </div>
        <div>
          <label className="block text-xs text-[#8892AA] mb-1">DV</label>
          <input readOnly value={nit ? calcDV(nit) : ''} className={`${inputBaseCls} bg-[#0A0E1A] cursor-not-allowed`}
                 {...register('digito_verificacion')} />
        </div>
      </div>
      <div className="mb-4">
        <label className="block text-xs text-[#8892AA] mb-1">Razón social *</label>
        <input placeholder="Mi Empresa S.A.S." className={inputBaseCls}
               {...register('razon_social', { required: 'Requerido' })} />
      </div>
      <div className="mb-4">
        <label className="block text-xs text-[#8892AA] mb-1">Régimen tributario</label>
        <select className={inputBaseCls} {...register('regimen_tributario')}>
      ...
      <div className="mb-4">
        <label className="block text-xs text-[#8892AA] mb-1">Tipo de empresa</label>
        <select className={inputBaseCls} {...register('tipo_empresa')}>
...
function StepActividad({ register }) {
  return (
    <div>
      <h2 className="text-xl font-black mb-1 text-[#F0F4FF]" style={{ fontFamily:'Syne,sans-serif' }}>Actividad económica</h2>
      <p className="text-sm text-[#8892AA] mb-5">Código CIIU y ubicación para ICA</p>
      <div className="mb-4">
        <label className="block text-xs text-[#8892AA] mb-1">Código CIIU *</label>
        <input placeholder="6201 — Desarrollo de sistemas informáticos" className={inputBaseCls}
               {...register('codigo_ciiu')} />
      </div>
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div>
          <label className="block text-xs text-[#8892AA] mb-1">Departamento</label>
          <select className={inputBaseCls} {...register('departamento')}>
            {['Antioquia','Bogotá D.C.','Valle del Cauca','Atlántico','Santander','Cundinamarca'].map(d => (
              <option key={d}>{d}</option>
            ))}
          </select>
        </div>
        <div>
          <label className="block text-xs text-[#8892AA] mb-1">Municipio</label>
          <input placeholder="Medellín" className={inputBaseCls} {...register('municipio')} />
        </div>
      </div>
      <div>
        <label className="block text-xs text-[#8892AA] mb-2">Responsabilidades tributarias</label>
        <div className="grid grid-cols-2 gap-2">
          {['IVA','Retención en la fuente','ReteICA','ICA','Impuesto al consumo'].map(r => (
            <label key={r} className="flex items-center gap-2 text-xs text-[#F0F4FF] cursor-pointer">
              <input type="checkbox" defaultChecked={['IVA','Retención en la fuente'].includes(r)}
...
function StepToken() {
  ...
  return (
    <div>
      <h2 className="text-xl font-black mb-1 text-[#F0F4FF]" style={{ fontFamily:'Syne,sans-serif' }}>Token DIAN</h2>
      <p className="text-sm text-[#8892AA] mb-5">Certificado digital para facturación electrónica</p>
      <div
        onClick={() => document.getElementById('token-file-onb')?.click()}
        className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all mb-4
          ${file ? 'border-[#00E5B0]/40 bg-[#00E5B0]/4' : 'border-white/10 hover:border-[#00E5B0]/25'}`}>
        <input id="token-file-onb" type="file" className="hidden"
               accept=".p12,.pfx,.cer" onChange={e => setFile(e.target.files[0])} />
        {file ? (
          <div>
            <div className="text-2xl mb-1">🔑</div>
            <div className="text-sm font-semibold text-[#00E5B0]">{file.name}</div>
            <div className="text-xs text-[#8892AA] mt-1">Listo para configurar</div>
          </div>
        ) : (
          <div>
            <div className="text-2xl mb-2">🔐</div>
            <div className="text-sm font-semibold text-[#F0F4FF] mb-1">Arrastra tu token aquí</div>
            <div className="text-xs text-[#8892AA]">.p12 · .pfx · .cer</div>
          </div>
        )}
      </div>
      <div className="flex items-center gap-2 bg-[#3D7BFF]/5 border border-[#3D7BFF]/10 rounded-xl p-3 text-xs text-[#8892AA]">
        💡 ¿Sin token? Puedes saltarte este paso y configurarlo después desde Ajustes.
      </div>
    </div>
  );
}

function StepPUC({ register }) {
  const [selected, setSelected] = useState('comercial');
  const TYPES = [
    { id:'comercial', label:'📊 PUC Comercial', desc:'Empresas de servicios y comercio' },
    { id:'constructor',label:'🏗️ PUC Constructor', desc:'Empresas constructoras' },
    { id:'financiero', label:'🏦 PUC Financiero', desc:'Entidades financieras' },
    { id:'esales',     label:'❤️ PUC ESALES',    desc:'Sin ánimo de lucro' },
  ];
  return (
    <div>
      <h2 className="text-xl font-black mb-1 text-[#F0F4FF]" style={{ fontFamily:'Syne,sans-serif' }}>Plan de cuentas</h2>
      <p className="text-sm text-[#8892AA] mb-5">Elige la plantilla PUC colombiana</p>
      <div className="grid grid-cols-2 gap-2 mb-4">
        {TYPES.map(t => (
          <button key={t.id} type="button" onClick={() => setSelected(t.id)}
                  className={`p-3 rounded-xl border text-left transition-all
                    ${selected===t.id ? 'border-[#00E5B0] bg-[#00E5B0]/8' : 'border-white/7 hover:border-white/15'}`}>
            <div className="text-xs font-semibold text-[#F0F4FF]">{t.label}</div>
            <div className="text-[10px] text-[#8892AA] mt-0.5">{t.desc}</div>
          </button>
        ))}
      </div>
      <div className="bg-[#00E5B0]/5 border border-[#00E5B0]/15 rounded-xl p-3 text-xs text-[#8892AA]">
        ✅ Se cargarán <strong className="text-[#F0F4FF]">+450 cuentas</strong> automáticamente.
        Puedes personalizar el PUC en cualquier momento.
      </div>
    </div>
  );
}
...
// ── HELPERS GLOBALES ──────────────────────────────────────────────────────────
export function PageHeader({ title, highlight, sub, children }) {
  return (
    <div className="flex items-center justify-between mb-6">
      <div>
        {sub && <div className="text-xs text-[#8892AA] mb-1">{sub}</div>}
...
export const inputBaseCls = `w-full bg-[#0D1525] border border-white/7 rounded-xl px-3 py-2.5 text-sm
  text-[#F0F4FF] outline-none transition-all placeholder:text-[#8892AA]/50
  focus:border-[#00E5B0]/40 focus:shadow-[0_0_0_3px_rgba(0,229,176,0.08)]`;

const selCls  = `w-full bg-[#0D1525] border border-white/7 rounded-xl px-3 py-2.5 text-sm
  text-[#F0F4FF] outline-none appearance-none cursor-pointer`;
